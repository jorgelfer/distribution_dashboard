  ...
  useEffect(() => {
    ...
    function init() {
      ...
      nodeEnter
        ...
        .attr("r", (d) => {
          d["radius"] = d.size
            ? d.size + props.originalNodeSize
            : props.originalNodeSize + 1;
          return d.radius + 5;
        })
      ...
      const updateNetwork = () => {
        ...
        nodeEnter.attr(
          "transform",
          (d) => `translate(${props.xScale(d.x)},${props.yScale(d.y)})`
        );
      ...
      function dragged(event, d) {
        ...
        net.nodes.forEach(function (otherNode) {
          let distance = Math.sqrt(
            Math.pow(otherNode.x - d.x, 2) + Math.pow(otherNode.y - d.y, 2)
          );
        ...
        if (foundOverlap === true) {
          d3.select(nodeDom).select("circle.background").style("opacity", 0.5);
        } else {
      ...
      function dragended(event, d) {
        ...
        d3.select(this).select("circle.background").style("opacity", 0);
      ...
      function deleteLink(event, d) {
        ...
        data.links = data.links.filter(
          (p) => !(p.source.id === d.source.id && p.target.id === d.target.id)
        );
        d3.selectAll(".node").remove();
        d3.selectAll(".link").remove();
        init();
